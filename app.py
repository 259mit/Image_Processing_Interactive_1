...
def main():
    ...
    ('Welcome','Image Analysis','Image Processing', 'Tools', 'Transformation')
    )
...
def imageanalysis():
  ...
  if st.button('Histogram Equalization'):
      ...
      st.text('Histogram equalization is a method in image processing')
      st.text('of contrast adjustment using the image\'s histogram.')
...
def transformations():
  ...
  X = st.text_input("Enter Width", "")
  Y = st.text_input("Enter Height", "")
  ...
  scale = st.text_input("Enter scale")
  x = st.text_input("Move horizontally by")
  y = st.text_input("Move vertically by")
